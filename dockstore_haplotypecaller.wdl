...
workflow haplotypeCaller {
  ...
  parameter_meta {
      ...
      mergeGVCFs_timeout: "Maximum amount of time (in hours) the task can run for."
      mergeGVCFs_cores: "The number of cores to allocate to the job."
      mergeGVCFs_overhead: "Java overhead memory (in GB). jobMemory - overhead == java Xmx/heap memory."
      mergeGVCFs_jobMemory: "Memory allocated to job (in GB)."
      mergeGVCFs_modules: "Required environment modules."
      callHaplotypes_timeout: "Maximum amount of time (in hours) the task can run for."
      callHaplotypes_cores: "The number of cores to allocate to the job."
      callHaplotypes_overhead: "Java overhead memory (in GB). jobMemory - overhead == java Xmx/heap memory."
      callHaplotypes_jobMemory: "Memory allocated to job (in GB)."
      callHaplotypes_refFasta: "The file path to the reference genome."
      callHaplotypes_modules: "Required environment modules."
      callHaplotypes_erc: "Mode for emitting reference confidence scores."
      callHaplotypes_intervalSetRule: "Set merging approach to use for combining interval inputs."
      callHaplotypes_intervalPadding: "The number of bases of padding to add to each interval."
      callHaplotypes_extraArgs: "Additional arguments to be passed directly to the command."
      callHaplotypes_dbsnpFilePath: "The dbSNP VCF to call against."
      ...
      splitStringToArray_lineSeparator: "line separator for intervalsToParallelizeBy. "
